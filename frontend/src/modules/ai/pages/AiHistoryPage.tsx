  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">AI History</p>
          <h1 className="display-font text-2xl font-semibold text-white md:text-3xl">Recent AI runs</h1>
        </div>
        <select
          value={filter}
        ...
        >
          {FILTER_OPTIONS.map((option) => (
            <option key={option} value={option}>
              {option === 'ALL' ? 'All tools' : AI_TOOL_LABELS[option] ?? option}
            </option>
          ))}
        </select>
      </header>
      ...
      {!loading && !error && filteredHistory.length === 0 && (
        <div className="rounded-3xl border border-white/10 bg-black/30 p-6 text-sm text-white/60">
          No AI history yet. Start a new analysis to see it here.
        </div>
      )}
          ...
          <div key={item.id} className="card-surface rounded-3xl border border-white/10 p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Input</p>
                <p className="mt-2 text-sm text-white/70 whitespace-pre-wrap">{item.inputText}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Output</p>
                <p className="mt-2 text-sm text-white/70 whitespace-pre-wrap">
                  {item.outputText ?? item.errorMessage ?? 'No output recorded.'}
                </p>
              </div>
            </div>
            <div className="mt-4 text-[11px] uppercase tracking-[0.2em] text-white/40">
              Tokens: {item.promptTokens + item.completionTokens} · Cost: ${item.costUsd.toFixed(2)}
            </div>
          </div>
        ))}
      </div>
