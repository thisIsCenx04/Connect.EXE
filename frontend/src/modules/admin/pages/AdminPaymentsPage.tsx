...
export function AdminPaymentsPage() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      try {
        ...
        if (!active) return
        setError('Kh?ng th? t?i ??n thanh to?n.')
      }
    }
    loadOrders()
    return () => {
  ...
  const handleReview = async (order: PaymentOrderAdmin, action: 'APPROVE' | 'REJECT', applyTo?: 'SUBSCRIPTION' | 'WALLET') => {
    ...
    try {
      ...
      setMessage('C?p nh?t ??n th?nh c?ng.')
    } catch {
      setError('Kh?ng th? duy?t ??n thanh to?n.')
    } finally {
      setLoadingId(null)
    }
  }
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Thanh to?n</p>
          <h2 className="display-font text-2xl font-semibold text-slate-900">Duy?t n?p ti?n th? c?ng</h2>
        </div>
        <select
          value={statusFilter}
        ...
        >
          ...
          <option value="PAID">?? duy?t</option>
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
        ...
        <div className="divide-y divide-slate-200">
          {orders.map((order) => (
            <div key={order.orderCode} className="grid grid-cols-[1.1fr_0.7fr_0.7fr_0.6fr_0.8fr_0.9fr] items-center gap-3 px-6 py-4 text-sm">
              ...
              <div>
                ...
                <div className="text-xs text-slate-400">{order.durationMonths} th?ng</div>
              </div>
          ...
          {orders.length === 0 && (
            <div className="px-6 py-6 text-sm text-slate-500">Kh?ng c? ??n thanh to?n.</div>
          )}
        </div>
      </div>
    </section>
